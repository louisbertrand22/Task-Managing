...
namespace TaskManager.API.Tests
{
    public class TasksControllerTests
    {
        ...
        public async Task GetTask_ReturnsTask_WhenTaskExists()
        {
            ...
            // Verify task exists in context before controller call
            var taskInContext = await context.Tasks.FindAsync(1);
            Assert.NotNull(taskInContext);
            Assert.Equal("Test Task", taskInContext.Title);
